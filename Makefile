...
QEMU_PROFILE_HOSTFWD := $(if $(filter xpra,$(NORMALIZED_PROFILES)),tcp:127.0.0.1:$(XPRA_PORT)-:$(XPRA_PORT),)
QEMU_HOSTFWD_EXTRA ?=
QEMU_HOSTFWD ?= $(strip $(QEMU_PROFILE_HOSTFWD)$(if $(QEMU_PROFILE_HOSTFWD),$(if $(QEMU_HOSTFWD_EXTRA),;,))$(QEMU_HOSTFWD_EXTRA))
...
profile-info:
	...
	@echo "QEMU clipboard: $(QEMU_CLIPBOARD)"
	@echo "QEMU host IP: $(QEMU_HOST_IP)"
...
.PHONY: run
run: all
	QEMU_HOSTFWD="$(QEMU_HOSTFWD)" QEMU_MEMORY="$(QEMU_MEMORY)" QEMU_KEYBOARD_LAYOUT="$(QEMU_KEYBOARD_LAYOUT)" QEMU_ACCEL="$(QEMU_ACCEL)" QEMU_CPU="$(QEMU_CPU)" QEMU_SMP="$(QEMU_SMP)" QEMU_CLIPBOARD="$(QEMU_CLIPBOARD)" QEMU_HOST_IP="$(QEMU_HOST_IP)" QEMU_APPEND="$(QEMU_APPEND)" sh scripts/run-qemu.sh "$(KERNEL_IMAGE)" "$(INITRAMFS)" "$(ROOTFS_IMAGE)"
...
.PHONY: run-gui
run-gui: all
	QEMU_GUI=1 QEMU_HOSTFWD="$(QEMU_HOSTFWD)" QEMU_MEMORY="$(QEMU_MEMORY)" QEMU_DISPLAY="$(QEMU_DISPLAY)" QEMU_VGA="$(QEMU_VGA)" QEMU_KEYBOARD_LAYOUT="$(QEMU_KEYBOARD_LAYOUT)" QEMU_ACCEL="$(QEMU_ACCEL)" QEMU_CPU="$(QEMU_CPU)" QEMU_SMP="$(QEMU_SMP)" QEMU_CLIPBOARD="$(QEMU_CLIPBOARD)" QEMU_HOST_IP="$(QEMU_HOST_IP)" QEMU_APPEND="$(QEMU_APPEND)" sh scripts/run-qemu.sh "$(KERNEL_IMAGE)" "$(INITRAMFS)" "$(ROOTFS_IMAGE)"
